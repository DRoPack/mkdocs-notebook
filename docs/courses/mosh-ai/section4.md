...
## Video 4.4 - Rendering the Messages
...
## Video 4.6 - Rendering Markdown text

- Install `React-Markdown`

- [x] [https://www.npmjs.com/package/react-markdown](https://www.npmjs.com/package/react-markdown)
...
## Video 4.7 - Adding a Typeing Indicator

```tsx title="client/src/ChatBot.tsx" hl_lines="5 10 34-35 20 45-51"
// ... existing logic

const ChatBot = () => {
 const [messages, setMessages] = useState<Message[]>([]);
 const [isBotTypeing, setIsBotTyping] = useState(false);
 const conversationId = useRef(crypto.randomUUID());
 const { register, handleSubmit, reset, formState } = useForm<FormData>();
   ...
   <div className="flex flex-col gap-3 mb-10">
    ...
    {isBotTypeing && (
     <div className="flex self-start gap-1 px-3 py-3 bg-gray-200 rounded-xl">
      <div className="w-2 h-2 rounded-full bg-green-800 animate-bounce [animation-delay:0s]"></div>
      <div className="w-2 h-2 rounded-full bg-green-800 animate-bounce [animation-delay:0.2s]"></div>
      <div className="w-2 h-2 rounded-full bg-green-800 animate-bounce [animation-delay:0.4s]"></div>
     </div>
    )}
   </div>
   // ...existing form logic
...
1. Changed to a `<div>`. Mosh's video is using a `<p>` element here but the console
   shows an error due to nested paragraph tags. This is happening
   because the `Markdown` element is compiled to a `<p>` tag as well.

## Video 4.8 - Auto-Scrolling to the Latest Message
